...
```{r}
results_train_bt <- car_train_1p %>% 
  mutate(mpg_log = log(mpg),
         lreg = predict(cars_lm_bt, car_train_1p),
         rfor = predict(cars_rf_bt, car_train_1p)) %>% 
  select(mpg, mpg_log, lreg, rfor, everything())
```
...
```{r}
yardstick::metrics(results_train_bt, truth = mpg_log, estimate = lreg)
```

```{r}
yardstick::metrics(results_train_bt, truth = mpg_log, estimate = rfor)
```

Let's test on the testing data
